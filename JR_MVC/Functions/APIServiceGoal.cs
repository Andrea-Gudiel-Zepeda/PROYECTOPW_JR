...
namespace JR_MVC.Functions
{
    public class APIServiceGoal
    {
        ...
        public static async System.Threading.Tasks.Task<IEnumerable<JR_DB.Goal>> GoalGetList(string accessToken)
        {
            var response = await GetListMethod("Goal/GetList", accessToken);
            if (response.StatusCode == System.Net.HttpStatusCode.OK)
            {
                return JsonConvert.DeserializeObject<IEnumerable<JR_DB.Goal>>(await response.Content.ReadAsStringAsync());
            }
            else
            {
        ...
        public static async System.Threading.Tasks.Task<JR_DB.GeneralResult> GoalSet(JR_DB.Goal object_to_serialize, string accessToken)
        {
            var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(object_to_serialize);
            var response = await SetMethod("Goal/Set", json_, accessToken);//httpClient.PostAsync(baseurl + "User/Set", content);
            if (response.StatusCode == System.Net.HttpStatusCode.OK)
            {
        ...
        public static async System.Threading.Tasks.Task<JR_DB.Goal> GetGoalByID(int id, string accessToken)
        {
            var response = await GetByIDMethod("Goal/GetByID", id, accessToken);
            if (response.StatusCode == System.Net.HttpStatusCode.OK)
            {
                return JsonConvert.DeserializeObject<JR_DB.Goal>(await response.Content.ReadAsStringAsync());
            }
            else
            {
                throw new Exception(response.StatusCode.ToString());
        ...
        public static async System.Threading.Tasks.Task<JR_DB.GeneralResult> GoalEdit(JR_DB.Goal object_to_serialize, int id, string accessToken)
        {
            var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(object_to_serialize);
            var response = await EditMethod("Goal/Edit", id, json_, accessToken);
            if (response.StatusCode == System.Net.HttpStatusCode.OK)
            {
                return JsonConvert.DeserializeObject<JR_DB.GeneralResult>(await response.Content.ReadAsStringAsync());
            }
        ...
        public static async System.Threading.Tasks.Task<JR_DB.GeneralResult> GoalDelete(int id, string accessToken)
        {
            //var json_ = Newtonsoft.Json.JsonConvert.SerializeObject(object_to_serialize);
            var response = await DeleteMethod("Goal/Delete", id, accessToken);//httpClient.PostAsync(baseurl + "Movies/Set", content);
            if (response.StatusCode == System.Net.HttpStatusCode.OK)
            {
                return JsonConvert.DeserializeObject<JR_DB.GeneralResult>(await response.Content.ReadAsStringAsync());
            }
